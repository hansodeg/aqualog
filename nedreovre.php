...
$anleggCsv = 'excel/nedreovre.csv';
$page_title = 'Nedre Øvre Renseanlegg';
$table_name = 'vannprover';
$anleggsid = 3;
$database = new Database();
$db = $database->connect();

$nedreOvreObject = new proveObject($db);
$table = $nedreOvreObject->showDB($anleggsid);
...
if($_SESSION['login'] == true){
	...
	if(isset($_POST['prove'])){
		...
		if($nedreOvreObject->saveData($ansattnr, $anleggsid, $opprettet, $signatur, $klor, $ph, $dpd1, $dpd3, $bundet_klor, $phenol)){
			echo "";
		}	
	}
	...
	WHERE anleggsid = 3 ORDER BY opprettet  DESC LIMIT 14" ;
...
<?php
	...
	if(isset($_POST['delete_table'])){
		$nedreOvreObject->deleteTable($table_name, $_SESSION['admin']);
	}
	//Run method when button is pressed
	if(isset($_POST['csvExport'])){
	...
	if(isset($_POST['deleteRow'])){
		$rowId = $_POST['chooseTestId'];
		$nedreOvreObject->deleteRow($rowId);
	}
?>
